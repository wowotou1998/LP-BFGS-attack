...
def main():
    ...
    print('-' * 19 + ' pytorch ' + '-' * 19)
    res1 = minimize(rosen, x0, method='bfgs', tol=1e-5, disp=True)

    print('\n' + '-' * 20 + ' scipy ' + '-' * 20)
    res11 = optimize.minimize(
    ...
    print('-' * 19 + ' pytorch ' + '-' * 19)
    res2 = minimize(rosen, x0, method='newton-exact', tol=1e-5, disp=True)

    print('\n' + '-' * 20 + ' scipy ' + '-' * 20)
    res22 = optimize.minimize(
        optimize.rosen, x0_np,
        method='trust-exact',
        jac=optimize.rosen_der,
